...
@section('content')
    ...
    <div class="container py-5">
        <div
            ...
            <a href="{{ route('pembeli.layanan-edukasi.create') }}" class="btn btn-primary"
                style="background-color: var(--primary-color); border: none;">
                <i class="fas fa-plus me-2"></i>
                <span class="d-none d-sm-inline">Booking Baru</span>
                            ...
                            <div class="booking-detail">
                                ...
                                <div class="d-flex justify-content-between align-items-center pt-3 border-top">
                                    <div class="d-flex flex-row">
                                        <a href="{{ route('pembeli.layanan-edukasi.show', $booking->id) }}"
                                            class="action-btn btn me-2"
                                            style="background-color: var(--primary-dark); color: var(--light-color);"
                                            title="Detail">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if ($booking->status == 'pending')
                                            <form action="{{ route('pembeli.layanan-edukasi.cancel', $booking->id) }}"
                                                method="POST" class="d-inline cancel-form">
                                                @csrf
                                                <button type="button" class="action-btn btn btn-danger cancel-btn"
                                                    title="Batalkan">
                                    ...
                                    <div>
                                        <small class="text-muted">Diperbarui:
                                            {{ $booking->updated_at->diffForHumans() }}</small>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            cancelButtons.forEach(function(btn) {
                btn.addEventListener('click', function() {
                    Swal.fire({
                        ...
                        text: "Booking ini akan dibatalkan dan tidak dapat diubah kembali.",
                        icon: 'warning',
